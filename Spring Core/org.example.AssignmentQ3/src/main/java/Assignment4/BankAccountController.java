...
@Service("service") public class BankAccountController implements ApplicationContextAware,BankAccountService  {
    private ApplicationContext context;
    @Override public double withdraw(long accountId, double balance){
        double newBalance = 0;
    ...
    }@Override public double getBalance(long accountId){
        BankAccountRepositoryImpl bankAccountRepository = (BankAccountRepositoryImpl) context.getBean("BankRepo");
        return bankAccountRepository.getBalance(accountId);
    }@Override public boolean fundTransfer(long fromAccount, long toAccount, double amont){
        BankAccountRepositoryImpl accountRepository = (BankAccountRepositoryImpl) context.getBean("BankRepo");
